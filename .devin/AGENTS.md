...
- **Memory persists**: state on disk (`.devin/loop_state.md` registry, `.devin/loop_state/<session_id>.md` per-session state, `.devin/session_state/<session_id>.json` machine state, and `.devin/knowledge_distill.md`), not context. See `MEMORY_PROTOCOL.md`.
...
3. **Read registry** — `.devin/loop_state.md` (<3KB). Inherit prior state:
   ...
   `handoff_letter.md`.
4. **Read knowledge layer** — `.devin/knowledge_distill.md` (<8KB). Anti-patterns.
5. **Read user profile** — `.devin/user_profile.md` (<2KB). Language, model tier,
   project type, custom red lines.
6. **Read handoff letter** — `.devin/handoff_letter.md` if it exists and `phase`
...
15. **Differential gap-scan** — scan 1-2 scope angles only. See `.devin/skills/gap-scan.md`.
...
| Knowledge | `.devin/knowledge_distill.md` | <8KB | required | Anti-patterns, reusable lessons. Grows by distillation only. |
...
If the same conflict recurs, update `.devin/knowledge_distill.md` and consider correcting memory.
...
3. Write project spirit / one-shot judgment to `.devin/handoff_letter.md` (not the canonical `.devin/canon/HANDOFF_LETTER.md`).
...
- `memory-audit` invokes `python .devin/scripts/memory_audit.py --session <session_id>`.
  ...
  `trigger + correct_action + counter`, merges valid entries into `.devin/knowledge_distill.md`,
...
## Links
- knowledge_distill: .devin/knowledge_distill.md
- handoff_letter: .devin/handoff_letter.md
...
| **Knowledge layer** | 15 | `knowledge_distill.md` exists, <8KB, ≥1 distilled entry | `ls .devin/knowledge_distill.md` + `wc -c` |
...
15. **No modifying `.devin/canon/HANDOFF_LETTER.md`.** Runtime judgment and project spirit go into `.devin/handoff_letter.md`. The canonical `HANDOFF_LETTER.md` is source-only and must not be edited by runtime scripts or sessions.
