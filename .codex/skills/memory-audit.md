---
name: memory-audit
description: "Use every 5 iterations, when scope changes, or before session end. Invokes .codex/scripts/memory_audit.py to review candidate memories, prune stale ones, and distills reusable patterns into knowledge_distill.md."
---
...
- Before writing the final `.codex/handoff_letter.md`.
...
2. The script does the following:
   ...
   - If all three exist, merge into `.codex/knowledge_distill.md` in the format:
     ```
   ...
   - If `.codex/knowledge_distill.md` exceeds 8KB, run a distillation pass:
     - merge duplicates
...
3. You may also read the audit output from the script and append any high-level judgment to `.codex/handoff_letter.md`.

## Output
```
## Memory audit
- script: python .codex/scripts/memory_audit.py --session <session_id>
- accepted: N
- discarded: N
