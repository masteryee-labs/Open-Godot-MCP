...
> 1. `python .codex/scripts/plan_dispatch.py --analyze --subtasks subtasks.json`
> 2. `python .codex/scripts/worktree.py create <worker_id>` (per worker)
...
Limits: do not ask the human. Do not modify .codex/canon/, AGENTS.md, or entry files.
        Uncertain → mark it, don't fabricate.
Comms: caveman full mode (see .codex/canon/CAVEMAN_PROTOCOL.md) — cut filler, keep code/paths/errors verbatim.
...
persona_path: [path to persona file, e.g. .codex/agents/personas/backend_architect.md]
```
