...
The caveman protocol (`.agents/canon/CAVEMAN_PROTOCOL.md`) compresses agent *output* (chat, reports). `comment_checker` extends that philosophy into *code comments* — the persistent text in the repo. Together: caveman protocol compresses every response; `comment_checker` compresses comments after code edits.
