...
> Thin wrapper over `.agents/canon/MEMORY_PROTOCOL.md`. Read at BOOT, write per-session state at the end of every iteration.
...
Follow `.agents/canon/MEMORY_PROTOCOL.md` for the full spec. Quick reference:
...
| Write every iteration | Update `.agents/loop_state/<session_id>.md` with date, phase, last_action, active GoalSpec, subtasks, deferred/human_required, `context_fill_pct`, `caveman_level`. Update `.agents/session_state/<session_id>.json` with `current_subtask`, `last_action`, `last_state_write`, `state_written: true`, `context_fill_pct`, `caveman_level`, `context_flags`, `owned_files`, `affected_files`, `tags`. Then call `python .agents/scripts/loop_memory_sync.py`. |
...
| Micro-memory | Run `memory-audit` skill every 5 iterations or scope change. It invokes `python .agents/scripts/memory_audit.py --session <session_id>`. |
