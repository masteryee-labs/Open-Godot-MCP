...
> Composes with `.agents/canon/VERIFICATION_PROTOCOL.md §Verbatim execution gates` — the maker
...
5. **Hunt the classic frauds**, in real-world frequency order:
   ...
   - **Non-code work**: judged by its domain's fraud table — load the matching adapter in
     `.agents/skills/domain-adapters/` and hunt ITS frauds (fabricated stats, stale figures, budget
...
Auditor (`.agents/skills/auditor.md`, `.agents/workers/AUDITOR.md`) runs every 5
