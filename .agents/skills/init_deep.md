...
- **Never duplicate the root canon.** Local files contain *local* context only. Global
  rules live in `.agents/canon/` and are synced by the deployer, not by init_deep.
- **Don't write AGENTS.md inside `.agents/canon/` or `core/assets/`.** Those are the
...
`init_deep` writes the **project context** layer — stable, structural, directory-bound. It complements (not replaces) the three-layer runtime memory (hot/knowledge/cold). See `.agents/canon/MEMORY_PROTOCOL.md`. It's the *spatial* memory: "where am I in the codebase" without reading 50 files.
