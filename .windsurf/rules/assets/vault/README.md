# .windsurf/rules/assets/vault/ ??The Local Asset Vault
...
|--------|-----------|------------|
| 1. Caveman Token Compression | `caveman_template.json` | `.windsurf/canon/CAVEMAN_PROTOCOL.md` |
| 2. Commander-Worker Hierarchy | `agency_framework.toml` | `.windsurf/COMMANDER.md` |
| 3. Loop Engineering & Vault Controls | `memory_mcp_schema.json` (SHA discipline) + `agency_framework.toml` (maker/checker) | `.windsurf/canon/LOOP_PROTOCOL.md` |
| 4. Deep Repository Memory | `memory_mcp_schema.json` | `.windsurf/canon/MEMORY_PROTOCOL.md` |
| 5. Sandbox Boundary Re-alignment | `strix_security_rules.json` (risk contracts + sandbox policy) | `.windsurf/canon/REDLINES.md` |

## How the deployer uses the vault

1. `scripts/sync.py` reads the canonical body from `.windsurf/canon/`.
...
- https://github.com/alchaincyf/nuwa-skill (nuwa-skill ??cognitive-diversity skill distillation factory, MIT)
  - Vendored at `.windsurf/rules/nuwa-skill/` with 3 example perspectives (munger/feynman/taleb)
  - See `.windsurf/rules/nuwa-skill/ATTRIBUTION.md` for full attribution
