...
|-------|------|-----|------|---------|
| Hot Registry | `.agents/loop_state.md` | <3KB | required | Generated by `.windsurf/scripts/loop_memory_sync.py`. Active sessions + recent 3 completed. |
...
- **Every iteration ends by writing the per-session files.** Non-negotiable. Skipping = red line.
  ...
  - Then call `python .windsurf/scripts/loop_memory_sync.py` to regenerate `.agents/loop_state.md` registry.
...
`.agents/loop_state.md` is a generated registry, not a hand-written file. `.windsurf/scripts/loop_memory_sync.py`
...
2. At the end of each iteration:
   ...
   - Call `python .windsurf/scripts/loop_memory_sync.py` to regenerate `.agents/loop_state.md`.
...
5. Call `python .windsurf/scripts/loop_memory_sync.py` to regenerate `.agents/loop_state.md` registry.
...
All loops follow the kickoff templates above. Steps: (1) check status/read state/run check command. (2) If problem found → read logs, fix, verify. (3) Re-check, write `.agents/loop_state/<session_id>.md` and `.agents/session_state/<session_id>.json`, then call `python .windsurf/scripts/loop_memory_sync.py` to update the registry; wait for cadence if `/loop`. State: §State contract. Maker/checker: CI/verifier checks; agent fixes. No self-approval.
...
1. **Register every session** before starting — `.windsurf/scripts/loop_memory_sync.py` writes the registry `.agents/loop_state.md` with `active_sessions: [{session_id, goal, status, tags, owned_files, last_heartbeat}]`.
