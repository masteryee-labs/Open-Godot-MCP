...
> Auto-generated from .windsurf/canon/. Do not edit this generated file; edit canon and re-run `python scripts/sync.py --canon`.



---

<!-- source: .windsurf/canon/CORE_CANON.md -->
...
- **Commander + workers**: main thread decides, dispatches, integrates. Workers scan/edit. See `.windsurf/COMMANDER.md`.
- **Parallel dispatch**: `.windsurf/scripts/plan_dispatch.py` (file ownership) + `.windsurf/scripts/worktree.py` (git worktree isolation). See `Docs/Agents/nuwa.md`.
- **Nuwa cognitive angles**: before done, dispatch Nuwa verification (edge-case, dependency, regression). Vendored at `.windsurf/rules/nuwa-skill/` (from alchaincyf/nuwa-skill, MIT). Three pre-distilled perspectives (Munger/Feynman/Taleb).
- **Memory persists**: state on disk (`.windsurf/loop_state.md` registry, `.windsurf/loop_state/<session_id>.md` per-session state, `.windsurf/session_state/<session_id>.json` machine state, and `.agents/knowledge_distill.md`), not context. See `MEMORY_PROTOCOL.md`.
...
|-------|----------|-------|---------|
| Canon (universal) | `.windsurf/canon/` | AHD deploys, project doesn't edit | BOOT_PROTOCOL, REDLINES |
| Project rules | `.windsurf/rules/` | Project owns, AHD doesn't touch | Game rendering rules, API conventions |
| Project profile | `.agents/user_profile.md` | Project owns | Red line summaries, never-read list, `project_rules_dir` pointer |

- `distill.py` **never overwrites** `.windsurf/rules/`. It is project-owned.
- `user_profile.md` has a `project_rules_dir` field (default: `.windsurf/rules/`) and optional `project_rules_index` field pointing to an index file.
...
<!-- source: .windsurf/canon/BOOT_PROTOCOL.md -->
...
2. **Ensure registry exists** — `.windsurf/loop_state.md` is the session registry.
   ...
   **One-time migration**: if shared state files (`user_profile.md`,
   ...
   are missing from `.agents/` but exist in `.windsurf/`,
...
<!-- source: .windsurf/canon/MEMORY_PROTOCOL.md -->
...
|-------|------|-----|------|---------|
| Hot Registry | `.windsurf/loop_state.md` | <3KB | required | Generated by `.windsurf/scripts/loop_memory_sync.py`. Active sessions + recent 3 completed. |
| Hot Session (human) | `.windsurf/loop_state/<session_id>.md` | <8KB | required for current session | Per-session GoalSpec, subtasks, notes, last action. Written by the AI each iteration. |
| Hot Session (machine) | `.windsurf/session_state/<session_id>.json` | <8KB | read only for audit/conflict | Machine-readable state: heartbeat, current subtask, owned/affected files, tags. Updated by hooks and `loop-memory`. |
| Knowledge | `.agents/knowledge_distill.md` | <8KB | required | Anti-patterns, reusable lessons. Grows by distillation only. |
| Cold | `.windsurf/loop_state_archive.md` + `.windsurf/loop_state_archive/<session_id>.md` | ∞ | grep only | Append-only event log + archived per-session markdown. Never read in full. |

## Write rules

- **Every iteration ends by writing the per-session files.** Non-negotiable. Skipping = red line.
  - Update `.windsurf/loop_state/<session_id>.md` with the GoalSpec, subtasks, last action, and notes.
  - Update `.windsurf/session_state/<session_id>.json` with `current_subtask`, `last_action`, `last_state_write`, `state_written: true`, `context_fill_pct`, `caveman_level`, and `context_flags`.
  - Then call `python .windsurf/scripts/loop_memory_sync.py` to regenerate `.windsurf/loop_state.md` registry.
...
`.windsurf/rules/chroma-hybrid-search/`.
...
& "$HOME\.deep-memory\.venv\Scripts\python" -m pip install -r .windsurf/rules/chroma-hybrid-search/requirements.txt

# Linux/macOS
python3 -m venv "$HOME/.deep-memory/.venv"
"$HOME/.deep-memory/.venv/bin/python" -m pip install -r .windsurf/rules/chroma-hybrid-search/requirements.txt
```

### Retrieval
```bash
# <PY> = ~/.deep-memory/.venv python (Windows: & "$HOME\.deep-memory\.venv\Scripts\python")
<PY> .windsurf/rules/chroma-hybrid-search/scripts/search.py \
  --query "task keywords" --limit 3 --min-score 0.35
```

### Writing cold notes
```bash
<PY> .windsurf/rules/chroma-hybrid-search/scripts/write_cold.py \
  --text "reusable takeaway" --tags "tag1,tag2" --project "agent-harness-deploy"
# then rebuild index:
<PY> .windsurf/rules/chroma-hybrid-search/scripts/update_db.py
```
...
### Conflict rule
Memory vs. current rules/files conflict → **current rules win**. Log conflict to `.windsurf/loop_state/<session_id>.md`.
...
(`.windsurf/workers/MEMORY_KEEPER.md`) to:

1. Call `python .windsurf/scripts/memory_audit.py --session <session_id>` to merge candidate memory.
2. Extract 1-3 reusable takeaways and write them to cold memory.
3. Write project spirit / one-shot judgment to `.agents/handoff_letter.md` (not the canonical `.windsurf/canon/HANDOFF_LETTER.md`).
4. Append a session-end marker to `.windsurf/loop_state_archive.md`.
...
- `post_tool_use.py` observes every tool call. If the same tool/command fails with the same error 2+ times,
  it writes a candidate entry to `.windsurf/session_state/<session_id>/candidate_memory.jsonl`:
  ```json
...
- `memory-audit` invokes `python .windsurf/scripts/memory_audit.py --session <session_id>`.
  It reads `.windsurf/session_state/<session_id>/candidate_memory.jsonl`, validates
...
`.windsurf/context_flags/<session_id>.json` is a per-session hot-path file that carries state across a single iteration:
```json
{
  "session_id": "s-...",
...
```
- `post_tool_use.py` writes `context_oversized: true` + `oversized_tool_calls_since_flag: 0` to `.windsurf/context_flags/<session_id>.json` when a tool response is oversized. It also prints a stderr directive to the agent.
...
- `context-compactor` reads `.windsurf/context_flags/<session_id>.json`, compacts, then clears `context_oversized: false` + resets counter to unblock the gate.
- `loop-memory` reads `.windsurf/context_flags/<session_id>.json` at the end of each iteration,
  copies `context_oversized` into `.windsurf/session_state/<session_id>.json`, and clears it.

## `loop_state.md` registry schema

`.windsurf/loop_state.md` is a generated registry, not a hand-written file. `.windsurf/scripts/loop_memory_sync.py`
produces it from `.windsurf/session_state/*.json` and `.windsurf/loop_state/<session_id>.md` front matter.
It must include:
```markdown
---
...
- session_archive: .windsurf/loop_state_archive.md
- session_archive_dir: .windsurf/loop_state_archive/
```
...
1. At the start of each iteration, set `.windsurf/session_state/<session_id>.json` `state_written` to `false`.
2. At the end of each iteration:
   - Update `.windsurf/loop_state/<session_id>.md` (GoalSpec, subtasks, last action, notes, caveman level).
   - Update `.windsurf/session_state/<session_id>.json` (current subtask, last action, `last_state_write`, `state_written: true`, `context_fill_pct`, `caveman_level`, `context_flags`, `owned_files`, `affected_files`, `tags`).
   - Call `python .windsurf/scripts/loop_memory_sync.py` to regenerate `.windsurf/loop_state.md`.
...
4. **Log to `.windsurf/loop_state/<session_id>.md`.** Record what expired and what was re-derived.
...
- Summarize completed work into a compact per-session state file (`.windsurf/loop_state/<session_id>.md`).
...
<!-- source: .windsurf/canon/LOOP_PROTOCOL.md -->
...
Every iteration:
1. Read `.windsurf/loop_state.md` registry (which sessions are active/completed).
2. Read `.windsurf/loop_state/<session_id>.md` for the active session (where did I get to?).
3. Do one unit of work.
4. Write `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json`
   (what did I do, what's next, what's still open).
5. Call `python .windsurf/scripts/loop_memory_sync.py` to regenerate `.windsurf/loop_state.md` registry.
...
If an agent has not produced output AND not written `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json` for **N** consecutive polling intervals (default N=2), the harness yanks it back:

1. **Re-inject the GoalSpec.** Re-read `.windsurf/loop_state/<session_id>.md`, restate the goal + current subtask.
2. **Force a state write.** Require `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json` update before any other action.
...
For user-driven loops: if no per-session state write and no done-declaration → prompt: "You went idle. Read `.windsurf/loop_state/<session_id>.md`, state current subtask, continue. If blocked, say what. If done, run verification."
...
- **State write is the heartbeat.** Writes `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json` = alive. Doesn't = idle or done — yank distinguishes.
...
|---|-------------------|---------|--------|
| 1 | `.windsurf/loop_state/<session_id>.md` or `.windsurf/session_state/<session_id>.json` not updated 2+ iterations | Skipping per-session state writes — loop blind | Force per-session state write first. 2nd round → red line, escalate. |
...
| 8 | `.windsurf/loop_state/<session_id>.md` exists but GoalSpec empty | BOOT incomplete | Stop. Re-run BOOT step 11. No work without GoalSpec for L/XL. |
...
| 14 | `.windsurf/context_flags/<session_id>.json.context_oversized = true` or `context_fill_pct > 70%` | Context degrading | Dispatch `context-compactor`. Offload large outputs. Lower `caveman_level`. |
...
- **Log signal fires to `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json`.** Record: which signal, when, action taken.
...
|----------|-----|----------------|---------------|
| **State persistence** | 20 | `.windsurf/loop_state.md` registry exists, <3KB; `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json` are written every iteration | `ls .windsurf/loop_state/<session_id>.md` + `ls .windsurf/session_state/<session_id>.json` + timestamps |
...
| **Memory safety** | 10 | No secrets in any layer; cold layer grep-only | `grep -r "key\|token\|password" .windsurf/` = nothing |
...
1. **At harness setup:** Run rubric manually. Score each category. Fix gaps. 2. **After major changes:** Re-score. 3. **Before unattended loops:** Must score ≥90. <90 = unattended mistakes. 4. **Track over time:** Log to `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json`. Dropping score = harness decay.
...
| 2 | Semantic drift: re-edits completed work | GoalSpec + completed-work scroll out of context | Edits file marked "done" or contradicts `knowledge_distill.md` | (a) Read `.windsurf/loop_state/<session_id>.md` every iteration. (b) Before editing: grep state+knowledge for filename. (c) Contradicts decision → STOP, re-read, confirm | 12 |
...
All loops follow the kickoff templates above. Steps: (1) check status/read state/run check command. (2) If problem found → read logs, fix, verify. (3) Re-check, write `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json`, then call `python .windsurf/scripts/loop_memory_sync.py` to update the registry; wait for cadence if `/loop`. State: §State contract. Maker/checker: CI/verifier checks; agent fixes. No self-approval.
...
Every kickoff must state: `Level: L1 (report-only) | L2 (assisted) | L3 (unattended)`. Log to `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json`. Track promotion/demotion history.
...
|----------|-------------|------------|
| **File race** | Two sessions edit same file | Worktree isolation per session (`.windsurf/scripts/worktree.py --session <session_id>`) |
| **CI race** | Two sessions push same branch | One session per branch. Tag commits. |
| **Duplicate work** | Two sessions fix same issue | Shared `.windsurf/loop_state.md` registry with `active_sessions[]` |
| **Resource contention** | Two sessions run expensive commands | Stagger cadences. |

### Coordination protocol

1. **Register every session** before starting — `.windsurf/scripts/loop_memory_sync.py` writes the registry `.windsurf/loop_state.md` with `active_sessions: [{session_id, goal, status, tags, owned_files, last_heartbeat}]`.
2. **Check before starting.** Read `.windsurf/loop_state.md` registry. If another `active_session` owns `owned_files`/`affected_files` overlapping the new task, ask the human whether to wait, continue, or serialize. **Never auto-resume.**
3. **Worktree isolation.** Each session's parallel workers run in their own worktree (`.windsurf/scripts/worktree.py create --session <session_id> <worker_id>`). No session edits the main checkout directly.
4. **Deregister on exit.** When a session completes or crashes, `.windsurf/scripts/loop_memory_sync.py` updates the registry and archives the per-session file to `.windsurf/loop_state_archive/<session_id>.md`.
...
Per triggered task:
  ...
  Step 4 — Write outcome to .windsurf/loop_state/<session_id>.md and
    .windsurf/session_state/<session_id>.json, then call
    python .windsurf/scripts/loop_memory_sync.py to update the registry.
...
<!-- source: .windsurf/canon/VERIFICATION_PROTOCOL.md -->
...
## In this harness
- `.windsurf/canon/VERIFICATION_PROTOCOL.md` — the rule, shipped to every tool.
- `.windsurf/workers/VERIFIER.md` — the Verifier worker (fresh context, checklist).
- `.windsurf/workers/AUDITOR.md` — the Auditor worker (fresh context, adversarial).
- `.windsurf/rules/fable-judge.md` — adversarial "done" gate; re-runs claimed verifications, hunts frauds, sweeps verbatim gate lines (INTENT/AUTH/TWINS/PENDING). Fires on every "done" declaration.
- `.windsurf/rules/harness-sensor.md` — the computational sensor (deterministic checks).
...
<!-- source: .windsurf/canon/CAVEMAN_PROTOCOL.md -->
...
- `post_tool_use.py` writes `context_oversized: true` + `oversized_tool_calls_since_flag: 0` to `.windsurf/context_flags/<session_id>.json` when a tool response is oversized. It also prints a stderr directive telling the agent to run `context-compactor` — most tools feed hook stderr back to the agent as feedback.
...
- `loop-memory` reads `.windsurf/context_flags/<session_id>.json` at the end of every iteration and updates `.windsurf/session_state/<session_id>.json` and `.windsurf/loop_state/<session_id>.md`.
- `.windsurf/loop_state.md` registry front matter must include:
  ```yaml
  context_fill_pct: <0-100 estimate>
  caveman_level: <light|compact|full|ultra|wenyan>
  active_session: s-...
  ```
- `.windsurf/loop_state/<session_id>.md` must include:
  ```yaml
  context_fill_pct: <0-100>
  caveman_level: <light|compact|full|ultra|wenyan>
  ```
...
<!-- source: .windsurf/canon/JUDGMENT_RUBRICS.md -->
...
| **Positive** | "About to run `rm -rf .windsurf/` — no .bak, no git tracking." |
...
| **Action +** | STOP. Read `.windsurf/loop_state/<session_id>.md` and `.windsurf/session_state/<session_id>.json`. Ask the human: "Session `s-xxx` was interrupted at `<current_subtask>`. Continue it, or start new?" |
...
| R-HE2 | Destructive without undo? | `rm -rf .windsurf/` — no .bak, no git | Overwrite CLAUDE.md — .bak created first | +: STOP. Ask human. No exceptions. |
...
<!-- source: .windsurf/canon/HANDOFF_LETTER.md -->
...
- `.windsurf/` = deployer dogfooding itself. Not a template for what gets synced.
...
<!-- source: .windsurf/canon/HARNESS_ENGINEERING.md -->
...
<!-- source: .windsurf/canon/REDLINES.md -->
...
3. **No canon drift.** Entry files (AGENTS.md, CLAUDE.md, instructions.md) are generated from `.windsurf/canon/`. Do not edit entry files directly. Edit canon, run sync.
...
13. **No reading all `loop_state/*.md` files at BOOT.** Read `.windsurf/loop_state.md` registry first, then only the one `.windsurf/loop_state/<session_id>.md` that matches the current task. Mass-reading session files is a red line.
14. **No secrets in tool log / session state / journal.** Never write keys, tokens, passwords, or API credentials into `.windsurf/session_state/`, `.windsurf/session_state/<session_id>/journal.jsonl`, or any tool log. Redact `command`/`counter` before logging.
15. **No modifying `.windsurf/canon/HANDOFF_LETTER.md`.** Runtime judgment and project spirit go into `.agents/handoff_letter.md`. The canonical `HANDOFF_LETTER.md` is source-only and must not be edited by runtime scripts or sessions.
