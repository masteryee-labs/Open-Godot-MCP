...
- **Memory persists**: state on disk (`.windsurf/loop_state.md` registry, `.windsurf/loop_state/<session_id>.md` per-session state, `.windsurf/session_state/<session_id>.json` machine state, and `.windsurf/knowledge_distill.md`), not context. See `MEMORY_PROTOCOL.md`.
...
| Knowledge | `.windsurf/knowledge_distill.md` | <8KB | required | Anti-patterns, reusable lessons. Grows by distillation only. |
...
If the same conflict recurs, update `.windsurf/knowledge_distill.md` and consider correcting memory.
...
3. Write project spirit / one-shot judgment to `.windsurf/handoff_letter.md` (not the canonical `.windsurf/canon/HANDOFF_LETTER.md`).
...
- `memory-audit` invokes `python .windsurf/scripts/memory_audit.py --session <session_id>`.
  ...
  `trigger + correct_action + counter`, merges valid entries into `.windsurf/knowledge_distill.md`,
...
## Links
- knowledge_distill: .windsurf/knowledge_distill.md
- handoff_letter: .windsurf/handoff_letter.md
...
| **Knowledge layer** | 15 | `knowledge_distill.md` exists, <8KB, ≥1 distilled entry | `ls .windsurf/knowledge_distill.md` + `wc -c` |
...
15. **No modifying `.windsurf/canon/HANDOFF_LETTER.md`.** Runtime judgment and project spirit go into `.windsurf/handoff_letter.md`. The canonical `HANDOFF_LETTER.md` is source-only and must not be edited by runtime scripts or sessions.
