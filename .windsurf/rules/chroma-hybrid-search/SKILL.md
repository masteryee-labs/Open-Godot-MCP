...
>
> **Repo-local script path** (this repo): `.windsurf/rules/chroma-hybrid-search/scripts/`.
...
<PY> -m pip install -r .windsurf/rules/chroma-hybrid-search/requirements.txt
...
### Hybrid (default)
```bash
<PY> .windsurf/rules/chroma-hybrid-search/scripts/search.py --query "spring animation tuning" --limit 3 --min-score 0.35
```

### Vector only
```bash
<PY> .windsurf/rules/chroma-hybrid-search/scripts/search.py --query "..." --mode vector --limit 3
```

### BM25 only
```bash
<PY> .windsurf/rules/chroma-hybrid-search/scripts/search.py --query "KEY_xxx" --mode bm25 --limit 3
```

### Scoped by skill or tag
```bash
<PY> .windsurf/rules/chroma-hybrid-search/scripts/search.py --query "session timeout" --skill backend-dev
<PY> .windsurf/rules/chroma-hybrid-search/scripts/search.py --query "config drift" --tag redis
```
