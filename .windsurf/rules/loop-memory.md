...
| Action | Rule |
|--------|------|
| Read at BOOT | `.windsurf/loop_state.md` registry (<3KB) + `.windsurf/knowledge_distill.md` (<8KB) + `.windsurf/user_profile.md` (<2KB) + `.windsurf/context_flags/<session_id>.json` if exists. Only read the matching `loop_state/<session_id>.md` and `session_state/<session_id>.json` for audit/conflict. |
