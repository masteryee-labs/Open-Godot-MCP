...
def get_config_root(root: Path) -> Path:
    ...
    In the source repo (``.windsurf/scripts/ahd_session.py``) the parent.parent
    ...
    Antigravity uses ``.windsurf/``, Claude Code uses ``.claude/``, Codex uses
    ``.codex/``, etc. The canonical text references ``.windsurf/`` as a placeholder;
...
def get_shared_state_root(root: Path) -> Path:
    """Return the canonical shared-state root (always ``.windsurf/``).

    Shared state files (user_profile.md, knowledge_distill.md,
    handoff_letter.md, context_quick_lookup.md) live at ``.windsurf/`` and are
    ...
    root but not in ``.windsurf/``, this function still returns ``.windsurf/``.
...
def resolve_shared_state_file(filename: str, root: Path) -> Path:
    ...
    Checks ``.windsurf/<filename>`` first (canonical location).
...
def get_session_id(data: Optional[Dict[str, Any]] = None, env_prefix: str = "AHD") -> str:
    ...
    3. file `.windsurf/session_state/current_session`
    4. UUID
    """
    data = data or {}
