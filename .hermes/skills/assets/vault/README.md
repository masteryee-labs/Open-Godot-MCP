# .hermes/skills/assets/vault/ ??The Local Asset Vault
...
|--------|-----------|------------|
| 1. Caveman Token Compression | `caveman_template.json` | `.hermes/canon/CAVEMAN_PROTOCOL.md` |
| 2. Commander-Worker Hierarchy | `agency_framework.toml` | `.hermes/agents/COMMANDER.md` |
| 3. Loop Engineering & Vault Controls | `memory_mcp_schema.json` (SHA discipline) + `agency_framework.toml` (maker/checker) | `.hermes/canon/LOOP_PROTOCOL.md` |
| 4. Deep Repository Memory | `memory_mcp_schema.json` | `.hermes/canon/MEMORY_PROTOCOL.md` |
| 5. Sandbox Boundary Re-alignment | `strix_security_rules.json` (risk contracts + sandbox policy) | `.hermes/canon/REDLINES.md` |

## How the deployer uses the vault

1. `scripts/sync.py` reads the canonical body from `.hermes/canon/`.
...
- https://github.com/alchaincyf/nuwa-skill (nuwa-skill ??cognitive-diversity skill distillation factory, MIT)
  - Vendored at `.hermes/skills/nuwa-skill/` with 3 example perspectives (munger/feynman/taleb)
  - See `.hermes/skills/nuwa-skill/ATTRIBUTION.md` for full attribution
