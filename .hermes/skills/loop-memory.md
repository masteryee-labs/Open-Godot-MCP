...
| Write every iteration | Update `.agents/loop_state/<session_id>.md` with date, phase, last_action, active GoalSpec, subtasks, deferred/human_required, `context_fill_pct`, `caveman_level`. Update `.agents/session_state/<session_id>.json` with `current_subtask`, `last_action`, `last_state_write`, `state_written: true`, `context_fill_pct`, `caveman_level`, `context_flags`, `owned_files`, `affected_files`, `tags`. Then call `python .hermes/scripts/loop_memory_sync.py`. |
