...
> Composes with `.hermes/canon/VERIFICATION_PROTOCOL.md §Verbatim execution gates` — the maker
...
5. **Hunt the classic frauds**, in real-world frequency order:
   ...
   - **Non-code work**: judged by its domain's fraud table — load the matching adapter in
     `.hermes/skills/domain-adapters/` and hunt ITS frauds (fabricated stats, stale figures, budget
...
Auditor (`.hermes/skills/auditor.md`, `.hermes/agents/workers/AUDITOR.md`) runs every 5
