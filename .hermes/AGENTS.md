...
17. **Differential gap-scan** — scan 1-2 scope angles only. See `.hermes/skills/gap-scan.md`.
...
|-------|------|-----|------|---------|
| Hot Registry | `.agents/loop_state.md` | <3KB | required | Generated by `.hermes/scripts/loop_memory_sync.py`. Active sessions + recent 3 completed. |
...
- **Every iteration ends by writing the per-session files.** Non-negotiable. Skipping = red line.
  ...
  - Then call `python .hermes/scripts/loop_memory_sync.py` to regenerate `.agents/loop_state.md` registry.
...
3. Write project spirit / one-shot judgment to `.agents/handoff_letter.md` (not the canonical `.hermes/canon/HANDOFF_LETTER.md`).
...
`.agents/loop_state.md` is a generated registry, not a hand-written file. `.hermes/scripts/loop_memory_sync.py`
...
2. At the end of each iteration:
   ...
   - Call `python .hermes/scripts/loop_memory_sync.py` to regenerate `.agents/loop_state.md`.
...
5. Call `python .hermes/scripts/loop_memory_sync.py` to regenerate `.agents/loop_state.md` registry.
...
1. **Register every session** before starting — `.hermes/scripts/loop_memory_sync.py` writes the registry `.agents/loop_state.md` with `active_sessions: [{session_id, goal, status, tags, owned_files, last_heartbeat}]`.
...
15. **No modifying `.hermes/canon/HANDOFF_LETTER.md`.** Runtime judgment and project spirit go into `.agents/handoff_letter.md`. The canonical `HANDOFF_LETTER.md` is source-only and must not be edited by runtime scripts or sessions.
