...
1. **BOOT** — follow `.hermes/canon/BOOT_PROTOCOL.md`.
...
5. **Verify** — per `.hermes/canon/VERIFICATION_PROTOCOL.md` (maker≠checker, fresh-context).
   Run `claim-grader` on every worker report before trusting it.
6. **Update state** — write `loop_state.md` every iteration (per `.hermes/canon/MEMORY_PROTOCOL.md`).
...
- Do not verify your own output — per `.hermes/canon/VERIFICATION_PROTOCOL.md` (S-tier exception aside).
